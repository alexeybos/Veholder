...
# Конфигурация
PROJECT_DIR="/mnt/c/Work/GitHub/SkillSmart/Veholder/veholder-app"
MAVEN_CMD=""
JAVA_VERSION="21"
...
# Функция для вывода сообщений с timestamp
log() {
...
setup_java() {
    ...
    if command -v java &> /dev/null; then
        ...
        if [ "$CURRENT_JAVA_VERSION" != "$JAVA_VERSION" ]; then
            ...
            #if command -v "java$JAVA_VERSION" &> /dev/null; then
            #    log "Переключаемся на Java $JAVA_VERSION..."
            #    export JAVA_HOME=$(update-alternatives --list java | grep "$JAVA_VERSION" | sed 's|/bin/java||' | head -n 1)
            #    export PATH="$JAVA_HOME/bin:$PATH"
            #    log "Переключено на Java $JAVA_VERSION"
            #else
            #    log "Java $JAVA_VERSION не установлена"
...
deploy_to_docker() {
	if [ -f "$LOCAL_JAR_PATH" ]; then
		...
		echo "Попытка остановить процесс внутри контейнера..."
		#docker exec veholder-app_app_1 sh -c "pkill -f java || true"
		#docker exec -d veholder-app_app_1 sh -c "java -jar $CONTAINER_JAR_PATH"
		echo "Рестарт  только контейнера с app"
		#docker-compose up --build app
		docker compose up --build app -d
		#docker exec veholder-app_app_1 sh -c "
			# Находим и убиваем Java процесс альтернативным способом
		#	JAVA_PID=\$(ps aux | grep '[j]ava.*veholder' | awk '{print \$2}')
		#	if [ ! -z \"\$JAVA_PID\" ]; then
		#		kill -9 \$JAVA_PID
		#	fi
		#	sleep 1
			# Запускаем новое приложение в фоне
		#	nohup java -jar $CONTAINER_JAR_PATH > /app/app.log 2>&1 &
		#"
...
deploy() {
	...
	#log "Запуск деплоя (заглушка)"
    #log "Функция деплоя будет реализована в будущем"
    
    # Здесь будет логика деплоя
    # Например: копирование JAR файла, запуск на сервере и т.д.
    
    #log "Деплой завершен (заглушка)"
    return 0
}
